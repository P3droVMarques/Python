#Funções é um bloco de código identificado por um nome e pode receber uma lista de parâmetros, esses parâmetros podem ou
#não ter valores padrões. Usar funções torna o código mais legível e possibilita o reaproveitamento de código. Programar
#baseado em funções, é o mesmo que dizer que estamos programando de maneira estruturada.
...
#Para retonar um valor , utilizamos a palavra reservada return. Toda função Python retornar none por padrão.
...
#Podemos combinar parâmetros obrigatórios com args e kwargs. Quando esses são definidos (*args e **kwargs), o método
...
#podemos atribuir funções a variáveis, passá-las como parâmetro para funções, usá-las como valores em estrutura de dados
#(lista, tuplas, dicionários, etc) e usar como valor de retorno para uma função(closures).
...
#Python trabalha com escopo local e global, dentro do bloco da função o escopo é local. Portanto alterações ali feitas
#em objetos imutáveis serão perdidas quando o método terminar de ser executado. Para usar objetos globais utilizamos a
